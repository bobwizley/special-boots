...
- **Exceção de registro:** os tipos de efeito de encantamento customizados (`ENCHANTMENT_ENTITY_EFFECT_TYPE`) são registrados sempre, independentemente do toggle. Um JSON de encantamento que referencia um tipo não registrado falha ao carregar e o encantamento desaparece do registro, o que quebraria botas que já o contêm. O toggle suspende o comportamento do efeito, não seu registro.
