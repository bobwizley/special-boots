...
The skills speak in terms of five canonical triage roles. In this repo those roles map **1:1 to Gitea labels** that already exist on `vctrtvfrrr/rootboot` — apply them with `tea issues edit <index> --add-labels <label>` (and `--remove-labels` for the previous one).
