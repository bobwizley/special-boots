...
Issues for this repo live in **Gitea** at `https://git.codelab.tec.br/vctrtvfrrr/rootboot`. Skills read and write them through the [`tea`](https://gitea.com/gitea/tea) CLI, which is already authenticated against `git.codelab.tec.br` (login `vctrtvfrrr`, set as default).

When run from inside the repo, `tea` discovers the repo from the git remote, so `--repo` is optional. Pass `--repo vctrtvfrrr/rootboot` explicitly when running from elsewhere.
...
tea issues create -t "..." -d "..." -L needs-triage -m "Alterar o plano de build"
